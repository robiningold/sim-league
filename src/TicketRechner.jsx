...
const TIER_START = [24, 14, 7, 3, 2, 1, 1, 1, 1];
...
export default function TicketRechner() {
  ...
  const [mode, setMode] = useState("topx");
  // Top-8-Verteilung auf dieselbe Ticketmenge wie "pro Win" skaliert: 24 + 16 + 2×8 + 4×6 = 80
  const [payoutPct, setPayoutPct] = useState(12.5);     // % des Feldes, das Tickets bekommt
  const [poolPerPlayer, setPoolPerPlayer] = useState(1); // Tickets, die jeder Teilnehmer in den Pool gibt
  ...
  const dep = [capacity, fill, tpw, rounds, entry, V, RC, payoutPct, poolPerPlayer, tierTickets, tPerWin, winBonus, fixedM, feePct, perTk];
  ...
  // Zielgrösse der Ausschüttung: jeder Teilnehmer wirft Tickets in den Pool.
  const pool = A.att * num(poolPerPlayer);
  const poolDelta = A.tpe - pool;
          ...
          <div className="grid md:grid-cols-2 gap-5">
            <div>
              ...
              {mode === "topx" ? (
                ...
                >
                  <NumField label="Bezahlte Plätze" value={payoutPct} min={2.5} max={100} step={2.5} onChange={setPayoutPct} suffix="%" />
                  {TIERS.map((t, i) => {
                  ...
                  <NumField label="Pool pro Teilnehmer" value={poolPerPlayer} min={0} max={5} step={0.5} onChange={setPoolPerPlayer} suffix="T" />
                  <Row label={"Pool · " + A.att + " Teilnehmer"} value={tk(pool)} />
                  <Row
                    label="Verteilt pro Event"
                    value={tk(A.tpe) + (poolDelta === 0 ? " · genau im Pool" : poolDelta > 0 ? " · " + tk(poolDelta) + " über Pool" : " · " + tk(-poolDelta) + " unter Pool")}
                    tone={poolDelta === 0 ? "p" : poolDelta > 0 ? "n" : "g"}
                    strong
                  />
                </Sec>
              ) : (
        ...
        <table style={{ width: "100%", fontSize: "12px", borderCollapse: "collapse" }}>
          <tbody>
            {[
              ...
              ["Ticket-Pool", tk(pool) + " (" + num(poolPerPlayer) + " T pro Teilnehmer)"],
